...
while [ 1 ]
do
	zenity --list --radiolist --title "Sys Admin Menu" --column "select" --column "Menu Item" FALSE "Display disk space" FALSE "Display users" FALSE "Display memory usage" FALSE "Exit"> $temp
	if [ $? -eq 1 ]
	then
		break
	fi
